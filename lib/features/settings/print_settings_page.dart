...
class PrintSettingsPage extends StatelessWidget {
            ...
            children: [
              ...
              Card(
                child: Padding(
                  padding: const EdgeInsets.all(16),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('三联单设置', style: TextStyle(fontWeight: FontWeight.w600)),
                    ...
                    if (settings.tripleFormEnabled) ...[
                      ...
                      // 联数选择
                      const Text('打印联数', style: TextStyle(fontWeight: FontWeight.w500)),
                      const SizedBox(height: 4),
                      Wrap(
                        spacing: 8,
                        children: AppConstants.tripleFormColors.asMap().entries.map((e) {
                          final index = e.key;
                          final color = e.value;
                          final isSelected = settings.tripleFormCopies.contains(index);
                          return FilterChip(
                            label: Text(color),
                            selected: isSelected,
                            onSelected: (selected) {
                              final copies = List<int>.from(settings.tripleFormCopies);
                              if (selected) {
                                copies.add(index);
                              } else {
                                copies.remove(index);
                      ...
                      // 打印模式
                      const Text('打印模式', style: TextStyle(fontWeight: FontWeight.w500)),
                      const SizedBox(height: 4),
              ...
              Card(
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 8),
                    Text('标准三联销售单，用于批发开单场景：', style: TextStyle(color: Theme.of(context).colorScheme.outline)),
                    const SizedBox(height: 4),
  ...
  Widget _buildColorLegend(BuildContext context, String title, String description) {
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text('- $description', style: TextStyle(color: Theme.of(context).colorScheme.outline)),
        ],
      ),
    );
  }
