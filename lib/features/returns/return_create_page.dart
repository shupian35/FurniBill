...
class _ReturnCreatePageState extends State<ReturnCreatePage> {
  String _returnType = 'sales_return';
  final _supplierNameCtrl = TextEditingController();
  final _reasonCtrl = TextEditingController();
  final _remarkCtrl = TextEditingController();
  final List<_ReturnItemData> _items = [];
  bool _saving = false;

  double get _totalAmount => _items.fold(0, (sum, i) => sum + (i.price * i.quantity));
          ...
          builder: (_, scrollCtrl) => Column(
            children: [
              Padding(
                ...
                child: Row(children: [
                  const Text('选择退货商品', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const Spacer(),
                  TextButton(
                    onPressed: () {
                      Navigator.pop(ctx);
  ...
  void _editItem(int index) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: qtyCtrl, decoration: const InputDecoration(labelText: '退货数量', border: OutlineInputBorder()), keyboardType: TextInputType.number),
          const SizedBox(height: 8),
          TextField(controller: priceCtrl, decoration: const InputDecoration(labelText: '单价', border: OutlineInputBorder()), keyboardType: TextInputType.number),
        ]),
        actions: [
  ...
  Future<void> _completeReturn() async {
    if (_items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('请添加退货商品')));
      return;
    }
    ...
    try {
      ...
      final items = _items.map((i) => ReturnOrderItem(
      ...
      final returnOrder = ReturnOrder(
        orderNo: orderNo,
        type: _returnType,
        supplierName: _supplierNameCtrl.text.trim().isNotEmpty ? _supplierNameCtrl.text.trim() : null,
        items: items,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('退货单已提交')));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('新建退货单')),
      body: Column(
        children: [
        ...
        Card(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('退货类型', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              const SizedBox(height: 8),
              SegmentedButton<String>(
                segments: const [
                  ButtonSegment(value: 'sales_return', label: Text('销售退货'), icon: Icon(Icons.shopping_bag_outlined)),
                  ButtonSegment(value: 'purchase_return', label: Text('采购退货'), icon: Icon(Icons.shopping_cart_outlined)),
                ],
                selected: {_returnType},
                onSelectionChanged: (v) => setState(() => _returnType = v.first),
              ),
              const SizedBox(height: 12),
              if (_returnType == 'purchase_return') ...[
                TextField(controller: _supplierNameCtrl, decoration: const InputDecoration(labelText: '供应商名称', border: OutlineInputBorder())),
                const SizedBox(height: 8),
              ],
              TextField(
                controller: _reasonCtrl,
                decoration: const InputDecoration(labelText: '退货原因', border: OutlineInputBorder(), hintText: '质量问题、规格不符等'),
              ),
            ]),
          ),
        ),
        const SizedBox(height: 12),
        const Text('退货商品', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
        const SizedBox(height: 8),
              ...
              subtitle: Column(
                ...
                children: [
                  if (i.spec != null && i.spec!.isNotEmpty)
                    Text(i.spec!, style: TextStyle(color: Theme.of(context).colorScheme.outline)),
                  Row(children: [
        ...
        Card(
          child: Padding(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(labelText: '备注', border: OutlineInputBorder(), hintText: '退货备注'),
              maxLines: 2,
            ),
          ),
        ),
        ...
        child: Row(children: [
          Expanded(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              AmountText(amount: _totalAmount, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)),
            ]),
          ),
          FilledButton(
            onPressed: _saving ? null : _completeReturn,
            style: FilledButton.styleFrom(minimumSize: const Size(120, 52)),
            child: Text(_saving ? '处理中...' : '确认退货', style: const TextStyle(fontSize: 16)),
          ),
        ]),
      ),
    );
