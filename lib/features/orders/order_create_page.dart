...
class _OrderCreatePageState extends State<OrderCreatePage> {
  ...
  Future<void> _completeOrder() async {
    ...
    try {
      ...
      final orderNo = isDraftEdit
          ? widget.draft!.orderNo
          : orderProvider.generateOrderNo();
      final items = _items
          .map(
            (i) => OrderItem(
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('开单成功')));
        // 弹出到订单列表并打开打印预览
        Navigator.pop(context, true);
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => PrintPreviewPage(order: order)),
        );
      }
    } catch (e) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('开单失败：$e')));
      }
    }
  }
                  ...
                  Row(
                    children: [
                      ...
                      if (i.discount < 1.0) ...[
                        const SizedBox(width: 8),
                        Text(
                          '${(i.discount * 100).toStringAsFixed(0)}折',
                    ...
                    SizedBox(
                      ...
                      child: TextField(
                        controller: TextEditingController(
                          text: '${(_orderDiscount * 100).toStringAsFixed(0)}',
                        ),
                        decoration: const InputDecoration(
                          suffixText: '%',
                          border: OutlineInputBorder(),
                ...
                Row(
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TextField(
                        controller: TextEditingController(
                          text: _roundOff.toStringAsFixed(2),
                        ),
                        decoration: const InputDecoration(
                          prefixText: '¥',
                          border: OutlineInputBorder(),
                        ...
                        keyboardType: TextInputType.number,
                        onChanged: (v) =>
                            setState(() => _roundOff = double.tryParse(v) ?? 0),
                      ),
                    ),
                  ],
                ),
                if (_roundOff <= 0) ...[
                  ...
                  TextButton(
                    onPressed: () {
                      // 抹去元以下
                      final frac =
                          _afterDiscount - _afterDiscount.floorToDouble();
              ...
              children: [
                ...
                if (_received > _receivable)
                  Padding(
                    ...
                    child: Text(
                      '找零：¥${(_received - _receivable).toStringAsFixed(2)}',
