    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 8),
          TextField(controller: priceCtrl, decoration: const InputDecoration(labelText: '采购单价', border: OutlineInputBorder()), keyboardType: TextInputType.number),
        ]),
        actions: [
  ...
  Future<void> _completeOrder() async {
    ...
    try {
      ...
      final order = PurchaseOrder(
        ...
        supplierPhone: _supplierPhoneCtrl.text.trim().isNotEmpty ? _supplierPhoneCtrl.text.trim() : null,
        items: items,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('采购单已提交')));
      ...
      children: [
        Card(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('供应商信息', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              const SizedBox(height: 8),
              TextField(controller: _supplierNameCtrl, decoration: const InputDecoration(labelText: '供应商名称', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: _supplierPhoneCtrl, decoration: const InputDecoration(labelText: '联系电话（选填）', border: OutlineInputBorder()), keyboardType: TextInputType.phone),
            ]),
          ),
        ),
        ...
        Card(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('付款信息', style: TextStyle(fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                initialValue: _paymentMethod,
                decoration: const InputDecoration(labelText: '付款方式', border: OutlineInputBorder()),
                items: const [
              ...
              const SizedBox(height: 8),
              TextField(
                controller: TextEditingController(text: _paidAmount > 0 ? _paidAmount.toString() : _totalAmount.toStringAsFixed(2)),
                decoration: const InputDecoration(labelText: '已付金额', border: OutlineInputBorder(), prefixText: '¥'),
        ...
        Card(
          child: Padding(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(labelText: '备注', border: OutlineInputBorder(), hintText: '采购备注'),
              maxLines: 2,
            ),
          ),
        ),
        ...
        child: Row(children: [
          Expanded(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_owingAmount > 0)
                Text('欠款：¥${_owingAmount.toStringAsFixed(2)}', style: const TextStyle(color: Colors.orange, fontSize: 12)),
            ]),
          ),
          FilledButton(
            onPressed: _saving ? null : _completeOrder,
            style: FilledButton.styleFrom(minimumSize: const Size(120, 52)),
            child: Text(_saving ? '处理中...' : '确认入库', style: const TextStyle(fontSize: 16)),
          ),
        ]),
      ),
    );
