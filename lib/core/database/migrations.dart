...
class Migrations {
  ...
  /// v9 瘦身：删除仓库/会员/盘点/分类/客户价/员工/库存日志 7 类周边表，
  /// 精简 products/customers/purchase_orders 列；新增 customers.note。
  /// 要求：SQLite >= 3.35（iOS 14+ / Android 10+ 自带满足）。
  static Future<void> v8ToV9(Database db) async {
    // 1) DROP 周边表（按依赖倒序）
    await db.execute('DROP TABLE IF EXISTS inventory_check_items;');
    await db.execute('DROP TABLE IF EXISTS inventory_checks;');
    await db.execute('DROP TABLE IF EXISTS customer_prices;');
    ...
    await db.execute('DROP TABLE IF EXISTS inventory_logs;');

    // 2) 精简 products 列
    await db.execute('ALTER TABLE products DROP COLUMN stock;');
    await db.execute('ALTER TABLE products DROP COLUMN min_stock;');
    await db.execute('ALTER TABLE products DROP COLUMN category_id;');
    await db.execute('ALTER TABLE products DROP COLUMN barcode;');
    await db.execute('ALTER TABLE products DROP COLUMN warehouse_id;');

    // 3) 精简 customers 列 + 新增 note
    await db.execute('ALTER TABLE customers DROP COLUMN tier;');
    await db.execute('ALTER TABLE customers DROP COLUMN credit_limit;');
    await db.execute('ALTER TABLE customers DROP COLUMN due_days;');
    await db.execute('ALTER TABLE customers DROP COLUMN total_owing;');
    await db.execute('ALTER TABLE customers ADD COLUMN note TEXT;');

    // 4) 重建 purchase_orders（去掉 warehouse_id 及 FK）
    //    ALTER TABLE DROP COLUMN 在某些 SQLite 版本上对带外键的列不可用，
    //    所以走“建新表 → 拷数据 → 删旧表 → 改名”四步法。
    await db.execute("""CREATE TABLE purchase_orders_new (
    ...
    await db.execute("""INSERT INTO purchase_orders_new
      (id, order_no, supplier_name, supplier_phone, items,
       total_amount, paid_amount, owing_amount, status, remark,
